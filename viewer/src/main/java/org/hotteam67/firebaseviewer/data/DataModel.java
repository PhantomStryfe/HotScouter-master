...
public class DataModel
{
    ...
    /**
     * Indicates if TBA Data failed to load
     */
    public static Boolean TBAError = false;
        ...
        try {
            ...
            TBAHandler.Matches(eventKey, new OnDownloadResultListener<List<TBAHandler.Match>>() {
                ...
                public void onComplete(List<TBAHandler.Match> matches) {
                    try {
                        Comparator<TBAHandler.Match> matchSorter
                                = TBAHandler.Match::compareTo;
                        Collections.sort(matches, matchSorter);
                        ...
                        FileHandler.Write(FileHandler.Files.VIEWER_MATCHES_FILE, e.toString());
                        Constants.Log(e);
                        TBAError = true;
                    }
                }
                ...
                public void onFail(String e) {
                    TBAError = true;
                    FileHandler.Write(FileHandler.Files.VIEWER_MATCHES_FILE, e);
                }
    ...
    {
        if (redTeamsQuals.isEmpty() || blueTeamsQuals.isEmpty()) {
            return null;
        }
